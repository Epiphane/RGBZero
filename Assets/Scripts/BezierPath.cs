...
public class BezierPath : MonoBehaviour {
    ...
    /// <summary>
    /// Get an angle along the path. At t=0 it will return the angle of the first point. At t=i it returns the angle of point i. At t=point count it will return the last point. At values in between, it returns the interpolated value.
    /// </summary>
    /// <param name="t">The t on the bezier spline</param>
    /// <returns>The interpolated angle</returns>
    public Vector3 GetAngleByT(float t) {
        if (t >= points.Count - 1) {
            // clamp to end
            return points[points.Count - 2].bezier.GetAngleAtTime(1);
        } else if (t <= 0) {
            // clamp to begin
            return points[0].bezier.GetAngleAtTime(0);
        } else {
            // position it
            int point1 = (int)t;
            return points[point1].bezier.GetAngleAtTime(t - point1);
        }
    }

    /// <summary>
    /// Get track angle along the path. At t=0 it will return the angle of the first point. At t=i it returns the angle of point i. At t=point count it will return the last point. At values in between, it returns the interpolated value.
    /// </summary>
    /// <param name="t">The t on the bezier spline</param>
    /// <returns>The interpolated angle</returns>
    public float GetTrackAngleByT(float t) {
        if (t >= points.Count - 1) {
            // clamp to end
            return points[points.Count - 2].trackAngle;
        } else if (t <= 0) {
            // clamp to begin
            return points[0].trackAngle;
        } else {
            // position it
            int point1 = (int)t;
            return points[point1].trackAngle * (1 - (t - point1))
                 + points[point1 + 1].trackAngle * (t - point1);
        }
    }
