...
public class Bezier
{
    ...
    public Vector3 GetAngleAtTime(float t) {
        this.CheckConstant();
        float t2 = t * t;
        float t3 = t * t * t;
        float x = this.Ax * t2 + this.Bx * 2 * t + this.Cx;
        float y = this.Ay * t2 + this.By * 2 * t + this.Cy;
        float z = this.Az * t2 + this.Bz * 2 * t + this.Cz;
        return new Vector3(x, y, z);
    }
